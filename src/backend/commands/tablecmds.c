			   ...
			   IndexStmt *stmt, bool is_rebuild, LOCKMODE lockmode)
{
	...
	{
		char *existing_index_name = get_rel_name(index_oid);
		...
		/*
		 * If there was no CONSTRAINT clause, assign the index's name. If we
		 * don't do it here then DefineIndex() will choose a system generated
		 * name, and we don't want that.
		 */
		if (stmt->idxname == NULL)
		{
			stmt->idxname = existing_index_name;
		}
		else if (strcmp(stmt->idxname, existing_index_name) != 0)
		{
